...
describe('Unitário: Geração de PDF (/generate-pdf)', () => {
  ...
  // Verifica se a API avisa o usuário (erro 400) quando faltam as informações da partida no payload
  ...
  // Testa o caminho feliz processando o buffer de dados e checando se a resposta traz headers de PDF
  it('returns pdf headers when valid body provided', async () => {
    ...
    const res = await request(app)
      ...
      .buffer()
      .parse((res, callback) => {
        // Trata adequadamente o binário recebido do PDFKit para que o Supertest possa checar o header
